...
export default function LedgerForm({action,fixed={},fields=[],label,disabled=false}:{action:string;fixed?:Record<string,string>;fields?:Field[];label:string;disabled?:boolean}) {
  ...
  return <form className="mt-4 flex flex-wrap items-end gap-3" onSubmit={async event=>{
    ...
    {fields.map(field=><label key={field.name} className="flex min-w-0 flex-col gap-2 text-xs text-slate-600">{field.label}{field.type==="number" ? <NumberInput key={`${field.name}-${field.value}`} required name={field.name} defaultValue={field.value} min={field.min} max={field.max} unit="تومان" className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm sm:w-44" /> : <input required name={field.name} type={field.type??"text"} defaultValue={field.value} min={field.min} max={field.max} maxLength={field.type==="text"||!field.type?500:undefined} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm sm:w-44" />}</label>)}
