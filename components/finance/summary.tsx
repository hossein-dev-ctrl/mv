...
export default function FinanceSummary({ totals }: { totals: ReturnType<typeof summarizeFinance> }) {
  ...
  return <>
    <dl className="my-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{items.map(([label, value, unit]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5"><dt className="text-xs text-slate-500">{label}<HelpTip text={financeHelp[label]}/></dt><dd className="mt-3 text-xl font-bold">{value.toLocaleString("fa-IR")} <span className="text-xs font-normal">{unit}</span></dd></div>)}</dl>
