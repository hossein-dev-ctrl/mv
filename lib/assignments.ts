...
export async function saveAssignment(actor:AssignmentActor,lessonId:string,input:z.infer<typeof assignmentInput>) {
 ...
 return prisma.$transaction(async tx=>{
  ...
  const lesson=await tx.lesson.findUnique({where:{id:lessonId},select:{status:true,section:{select:{course:{select:{id:true,slug:true,status:true,teacherId:true}}}}}});
  ...
  const saved=await (current?tx.assignment.update({where:{id:current.id},data:{...data,version:{increment:1}}}):tx.assignment.create({data:{lessonId,...data}}));
  if(input.published&&lesson.status==='PUBLISHED'&&lesson.section.course.status==='PUBLISHED'){
   const course=lesson.section.course;
   await notifyCourse(tx,course.id,{title:current?.published?'تکلیف دوره به‌روز شد':'تکلیف جدید در دورهٔ شما',body:input.title,href:`/courses/${course.slug}`,eventKey:`assignment:${saved.id}:v${saved.version}`});
  }
  return saved;
 });
}
export async function submitAssignment(actor:AssignmentActor,lessonId:string,input:z.infer<typeof submissionInput>) {
 ...
 return prisma.$transaction(async tx=>{
  ...
  const submitted=await tx.submission.create({data:{assignmentId:assignment.id,enrollmentId:enrollment.id,attempt:(latest?.attempt??0)+1,answer:input.answer,projectUrl:input.projectUrl||null,assignmentTitle:assignment.title,assignmentInstructions:assignment.instructions}});
  await notifyUsers(tx,[assignment.lesson.section.course.teacherId],{title:'پاسخ جدید تکلیف',body:assignment.title,href:`/teacher/courses/${enrollment.courseId}/assignments`,eventKey:`submission:${submitted.id}:created`});
  return submitted;
 });
}
export async function reviewSubmission(actor:AssignmentActor,id:string,input:z.infer<typeof reviewInput>) {
 ...
 return prisma.$transaction(async tx=>{
  ...
  const submission=await tx.submission.findUnique({where:{id},include:{enrollment:true,assignment:{include:{lesson:{include:{section:{include:{course:true}}}}}}}});
  ...
  const reviewed=await tx.submission.update({where:{id},data:{status:input.action==='grade'?'GRADED':'REVISION',score:input.action==='grade'?input.score:null,feedback:input.feedback,reviewedAt:new Date(),reviewedBy:actor.id}});
  await notifyUsers(tx,[submission.enrollment.userId],{title:input.action==='grade'?'نمرهٔ جدید در کارنامه':'پاسخ تکلیف نیاز به اصلاح دارد',body:submission.assignment.title,href:`/dashboard/courses/${submission.enrollment.courseId}/grades`,eventKey:`submission:${id}:review`});
  return reviewed;
 });
}
