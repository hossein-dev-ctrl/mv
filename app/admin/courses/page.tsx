...
export default async function AdminCoursesPage() {
  ...
  return <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2">{courses.map(course => <article key={course.id} className="rounded-2xl border border-slate-200 bg-white p-6">
      ...
      <div className="flex flex-wrap gap-4 text-sm"><Link href={`/teacher/courses/${course.id}/assignments`} className="text-indigo-700">تکلیف‌ها و ارزیابی</Link><Link href={`/courses/${course.slug}`} className="text-indigo-700">مشاهدهٔ دوره از دید کاربر</Link><Link href={`/teacher/courses/${course.id}/interests`} className="text-indigo-700">متقاضیان پیش‌ثبت‌نام</Link><Link href={`/teacher/courses/${course.id}/students`} className="font-medium text-indigo-600">مشاهدهٔ پیشرفت دانش‌آموزان</Link><Link href={`/teacher/courses/${course.id}`} className="text-slate-600">مدیریت دوره</Link></div>
    </article>)}</div>
  </main>;
}
