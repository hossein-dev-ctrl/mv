...
export default async function SettlementsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}) {
 ...
 const status=typeof query.status==='string'&&['PROCESSING','PAID','REJECTED','DISPUTED','ALL'].includes(query.status)?query.status:'REQUESTED';
 const settings=await prisma.financeSettings.findUnique({where:{id:'main'}});
 const where:Prisma.PayoutWhereInput={...(status==='ALL'?{}:status==='DISPUTED'?{status:'PAID',disputedAt:{not:null},receivedAt:null,reviewedAt:null}:{status}),...(process.env.NODE_ENV==='production'?{teacher:{demoBatchId:null}}:{})};
 ...
 const payouts=await prisma.payout.findMany({where,orderBy:{requestedAt:'desc'},take:20,skip:(page-1)*20,include:{reviews:{orderBy:{createdAt:"asc"}},teacher:{select:{name:true,email:true}}}});
 ...
 return <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
  <Link href="/admin/finance" className="panel-action panel-action-indigo"><ThemeIcon name="arrow" className="me-2 h-4 w-4"/>بازگشت به مالی کل</Link><h1 className="my-5 text-2xl font-bold">تسویه و بازپرداخت</h1>
  ...
  <nav aria-label="وضعیت درخواست برداشت" className="my-5 flex flex-wrap gap-2">{[['REQUESTED','در انتظار'],['PROCESSING','در حال واریز'],['DISPUTED','عدم دریافت'],['PAID','واریزشده'],['REJECTED','ردشده'],['ALL','همه']].map(([value,label])=><Link scroll={false} key={value} href={`/admin/settlements?status=${value}`} aria-current={status===value?'page':undefined} className={`rounded-xl px-4 py-2 text-sm ${status===value?'bg-indigo-600 text-white':'bg-white text-slate-600'}`}>{label}</Link>)}</nav>
  ...
  <nav className="my-5 flex justify-center gap-4 text-sm" aria-label="صفحه‌بندی درخواست‌ها">{page>1&&<Link scroll={false} href={url(page-1,paymentPage)} className="panel-action panel-action-slate"><ThemeIcon name="arrow" className="me-2 h-4 w-4"/>قبل</Link>}<span>{page.toLocaleString('fa-IR')} از {pages.toLocaleString('fa-IR')}</span>{page<pages&&<Link scroll={false} href={url(page+1,paymentPage)} className="panel-action panel-action-slate"><ThemeIcon name="arrow" className="me-2 h-4 w-4"/>بعد</Link>}</nav>
  ...
  <nav className="my-5 flex justify-center gap-4 text-sm" aria-label="صفحه‌بندی پرداخت‌ها">{paymentPage>1&&<Link scroll={false} href={url(page,paymentPage-1)} className="panel-action panel-action-slate"><ThemeIcon name="arrow" className="me-2 h-4 w-4"/>قبل</Link>}<span>{paymentPage.toLocaleString('fa-IR')} از {paymentPages.toLocaleString('fa-IR')}</span>{paymentPage<paymentPages&&<Link scroll={false} href={url(page,paymentPage+1)} className="panel-action panel-action-slate"><ThemeIcon name="arrow" className="me-2 h-4 w-4"/>بعد</Link>}</nav>
 </main>;
}
