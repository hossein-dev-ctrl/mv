...
export default async function SettlementsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}) {
 ...
 return <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
  ...
  <nav className="my-5 flex justify-center gap-4 text-sm" aria-label="صفحه‌بندی درخواست‌ها">{page>1&&<Link scroll={false} href={url(page-1,paymentPage)}>قبل</Link>}<span>{page.toLocaleString('fa-IR')} از {pages.toLocaleString('fa-IR')}</span>{page<pages&&<Link scroll={false} href={url(page+1,paymentPage)}>بعد</Link>}</nav>
  ...
  <nav className="my-5 flex justify-center gap-4 text-sm" aria-label="صفحه‌بندی پرداخت‌ها">{paymentPage>1&&<Link scroll={false} href={url(page,paymentPage-1)}>قبل</Link>}<span>{paymentPage.toLocaleString('fa-IR')} از {paymentPages.toLocaleString('fa-IR')}</span>{paymentPage<paymentPages&&<Link scroll={false} href={url(page,paymentPage+1)}>بعد</Link>}</nav>
 </main>;
}
