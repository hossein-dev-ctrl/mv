...
test('demo cleanup refuses outside dependencies before any deletion',async()=>{
 ...
 const tx={submission:{deleteMany:async()=>({count:0})},courseInterest:{count:async()=>0},$queryRaw:async()=>[],demoBatch:{findUnique:async()=>({id:'batch',teacherId:'t'})},user:{findMany:async()=>[{id:'t'}]},course:{findMany:async()=>[{id:'c',teacherId:'t'}],count:async()=>1},payment:{count:async()=>0},enrollment:{count:async()=>0}};
...
test('demo cleanup scopes every deletion and keeps the global minimum',async()=>{
 ...
 const tx={submission:{deleteMany:async()=>({count:0})},courseInterest:{count:async()=>0},$queryRaw:async()=>[],demoBatch:{findUnique:async()=>({id:'batch',teacherId:'t'}),delete:remove('batch')},user:{findMany:async()=>[{id:'t'},{id:'s'}],deleteMany:remove('users')},course:{findMany:async()=>[{id:'c',teacherId:'t'}],count:async()=>0,deleteMany:remove('courses')},payment:{count:async()=>0,findMany:async()=>[{id:'p'}],deleteMany:remove('payments')},enrollment:{count:async()=>0,deleteMany:remove('enrollments')},paymentRefund:{deleteMany:remove('refunds')},paymentCost:{deleteMany:remove('costs')},payout:{findMany:async()=>[{id:'demo-payout'}],deleteMany:remove('payouts')},notification:{deleteMany:remove('notifications')}};
 await demo(tx).clearFinanceDemo('admin');assert.deepEqual(calls.payments,{id:{in:['p']}});assert.equal(calls.users.demoBatchId,'batch');assert.deepEqual(calls.payouts,{teacherId:{in:['t','s']}});assert.deepEqual(calls.batch,{id:'batch'});assert.deepEqual(calls.notifications,{OR:[{eventKey:{startsWith:'payout:demo-payout:'}}]});
 }finally{process.env.NODE_ENV=before;}
});
